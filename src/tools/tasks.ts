  ...
  server.registerTool(
    "mono_list_tasks",
    {
      ...
      inputSchema: z.object({
        ...
        type: z.enum(["epic", "story", "task", "bug"]).optional().describe("Filter: epic, story, task, bug. Sub-tasks are regular tasks with parent_id."),
  ...
  server.registerTool(
    "mono_create_task",
    {
      ...
      inputSchema: z.object({
        ...
        type: z.enum(["epic", "story", "task", "bug"]).optional().describe("Type: epic, story, task, bug (default: task). For sub-tasks set type='task' and parent_id to the parent task UUID."),
        ...
        parent_id: z.string().optional().describe("Parent task UUID — makes this task appear as a sub-task under the parent. Sub-tasks are not a separate type."),
  ...
  server.registerTool(
    "mono_update_task",
    {
      ...
      inputSchema: z.object({
        ...
        type: z.enum(["epic", "story", "task", "bug"]).optional().describe("New type: epic, story, task, bug"),
